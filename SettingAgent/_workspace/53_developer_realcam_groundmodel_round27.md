...
| jpeg완료 → getptzfpos **시작** | 3~335ms (중앙값 31ms) |

정렬 전량: `36 38 39 39 45 50 51 52 54 55 55 57 60 60 60 61 62 62 63 67 69 72 74 77 81 84 98 273 368`

→ **인용값은 「36~368ms (n=29, 중앙값 60ms)」로 한다.** `33~51ms` 로 줄여 적지 않는다(다른 양이다). 내 초고의 `72~368ms` 도 **철회**한다. (27-B 는 중앙값을 ≈61ms 로 적었다 — 내 셈으로는 15번째 값 = **60**. 차이는 무시할 수준이나 내 실측값을 적는다.)

- ★ **방법론 한계 — 그리고 그 직접 증거.** 패킷 로그의 인접이 **같은 `snapshot()` 호출임을 증명하지 못한다.** 이건 추상적 우려가 아니다: 필터 없이 인접 쌍을 전부 뽑으면 n=46 이 되고 상위 3건이 `1652656 / 2101769 / 25124711`ms — **최대 약 7시간**이다. 세션이 갈린 jpeg 와 몇 시간 뒤 getptzfpos 가 짝지어진 것이다. 그래서 **간격 1000ms 미만만** 같은 호출로 간주해 29건을 얻었다(1000ms 초과 17건은 전부 16초~7시간으로 뚜렷이 분리돼 경계 선택이 결과를 바꾸지 않는다).
