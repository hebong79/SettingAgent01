...
8. **★★ 못 잡는 것은 「자기 것」이 아니라 「다시 읽기」다.** 오류 1~7 을 놓칠 때 우리는 문서를 **읽고** 있었고, 8·9 를 잡을 때는 **수치별로 grep 해 전수 대조**하고 있었다. 사람이 아니라 **방법**의 차이다.
   ...
   - **따름정리 ② — 스윕에도 절차가 필요하다(경계·범위를 주지 않으면 거짓양성).** 실증: 원장 행수를 확인하려고 문서 전체에 `grep -c "^| [0-9] |"` 를 돌려 **17** 을 얻었다. 실제 원장은 **9행**이고 나머지 8 은 다른 두 표(§0 요약 5 · §5 [D] 3)였다. 표 범위를 `awk '/^\| # \| 낸 쪽/,/^$/'` 로 한정해야 9 가 나온다. 27-B 도 `7건` 스윕에서 `27건`(≤98ms 표본 수)이 걸려 단어경계 `(?<![0-9])7건` 로 재검사했다. **스윕 결과는 「세었다」가 아니라 「무엇을 세었는지」와 함께 읽어야 한다** — 규칙 3(모집단)의 스윕판이다.
     - ★ **기각도 기계로 하라.** 거짓양성을 **읽어서 기각**하면 스윕을 돌려 놓고 판정은 「다시 읽기」로 한 셈이라 **규칙 8 이 스스로 무력화된다**("스윕을 돌렸다"는 알리바이로 전락). 기각은 **경계를 좁히거나 범위를 한정해 재실행**하는 방식이어야 한다. 위 17·16 을 나는 처음에 읽어서 기각했고, 그래서 `awk` 재실행으로 다시 판정했다.
     - ★★ **판정문 자체도 검증 대상이다 — 내 것은 세 번 만에 옳아졌다.** 27-B 정식화("산문으로 적으면 지켜지지 않는다 → **실행 가능한 판정문(exit code)** 으로 만들어라")를 받아 보고서·코드 정합 검사를 exit code 판정으로 짰더니 **두 번 연속 거짓 FAIL** 이 났다:
       | 판 | 결과 | 원인 |
       |---|---|---|
       | v1 | FAIL | 「`roi.auto.apply` **미호출**」이라 적힌 **주석**에 정규식이 매칭 |
       | v2 | FAIL | **파일 단위 논리곱**(`writeFile` ∧ `PtzCamRoi`) — 쓰기는 `reports/` 로, `PtzCamRoi` 는 읽기인데 묶어 버림 |
       | v3 | **PASS (exit 0)** | 주석 제거 + **문장 단위**(쓰기 호출의 *인자*만 검사, `PtzCamRoi` 는 읽기 줄에만 등장하는지) |
       → **v1·v2 의 「FAIL」을 액면가로 받았다면 있지도 않은 정본 쓰기 위반을 보고했을 것이다.** 이는 「PASS 를 액면가로 받는 것」의 거울상이다. **기계 판정은 「다시 읽기」보다 낫지만 무오류가 아니다 — 판정문에도 경계·범위 규율이 그대로 필요하다.** 실제 통과 결과: `원장 9건(선언 9) · 교훈 9개 · 확정값 3/3 · 쓰기호출 3건 전부 reports/ · PtzCamRoi 읽기전용`.
     - ★★ **판정문이 「실패할 수 있는지」를 확인하라 — 항상 PASS 하는 검사는 무가치하다.** 나는 이 절차를 27-B 에게 제안해 놓고 **정작 내 것에는 돌리지 않았다**(이 회차 「규율을 말한 문장 안의 규율 위반」 다섯 번째, 이번엔 내 것). 보고서 사본에 위반을 심어 실측했다:

       | 변이 | 심은 위반 | 결과 |
       |---|---|---|
       | 대조군 | 없음 | **PASS** (exit 0) |
       | 1 | 교훈 번호 결번 `7.`→`9.` | **FAIL** ✔ |
       | 2 | 원장 선언값 위조 `9건`→`7건` | **FAIL** ✔ |
       | 3 | 확정값 `60`→`61`(= 오류 9 재현) | **FAIL** ✔ |
       | 4 | 원장 행 **조용히 삭제** | **FAIL** ✔ (원장엔 선언 개수가 있어 잡힌다) |
       | **5** | **교훈 행 삭제 + 번호 1~8 연속** | ⚠ **PASS — 못 잡는다** |
       | **6** | **원장 행 삭제 + 선언값도 8 로 함께 위조** | ⚠ **PASS — 못 잡는다** |

       - **사각지대 ⓐ(변이 5)**: 교훈 절에는 **선언 개수가 없어** 연속성만 본다 → 항목이 사라져도 번호가 1..n 이면 통과한다. 27-B 가 자기 원장에서 자백한 축과 **같은 것**이며, 내 쪽은 교훈 절에 있다. **고치지 않고 미검증으로 남긴다** — 마감 직전 구조 변경은 새 오류의 자리다.
       - ★ **사각지대 ⓑ(변이 6) — 이건 선언 개수를 추가해도 못 고친다.** 삭제와 선언값 수정이 **함께** 일어나면 문서는 **자기정합적**이다. **내부 정합 검사는 「불일치」만 잡을 수 있고 「자기정합적 삭제」는 원리적으로 못 잡는다.** 이를 잡으려면 **문서 밖의 기준**(직전 커밋과의 `git diff`)이 필요하다 — 검사를 더 촘촘히 해서 될 일이 아니다. 정합검사 exit 0 은 **「내부 모순 없음」이지 「내용 보존됨」이 아니다.**
     - ★ **부정 단언은 「양성 대조」로 공허하지 않음을 보여라.** 「`roi.auto.apply` 호출 없음」·「정본 쓰기 없음」 같은 검사는 **0건이 곧 통과**라, 패턴이 틀려도 조용히 통과한다. 실제 위반을 심어 FAIL 을 확인했다: **A**(`roi.auto.apply` 호출 주입) → **exit 1** ✔ / **B**(`writeFileSync("data/Place01/PtzCamRoi.json"…)` 주입) → **exit 1** ✔ / 대조군(원본) → exit 0. 관련해 27-B 가 **거짓 음성**도 실측했다(`재현 11/11` 로 훑어 0건 → 본문은 `**11/11 일치.**` 라 패턴 불일치였을 뿐). → **「0건이니 없다」도 「N건이니 있다」만큼 위험하다 — 0 이 나오면 패턴부터 의심하라.**
     - ★ **개수만 세지 말고 번호 연속성도 기계로 봐라.** 항목을 삽입·삭제하면 **개수가 맞아도 번호가 중복·결번**될 수 있다(27-B 지적 — 나도 원장에서 개수만 세고 있었다). 실측: 원장 `1 2 3 4 5 6 7 8 9` · 교훈 `1.~9.` **연속 통과**(`sort -n` 후 인접차 1 판정).
