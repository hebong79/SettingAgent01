...
6. **★ 「자기 것은 못 잡는다」는 틀렸다 — 못 잡는 것은 *다시 읽기*다.**
   ...
   **따름정리 ②(내 스윕이 방금 보여준 것)**: 기계 스윕도 **패턴에 경계가 없으면 거짓양성**을 낸다. 내가 `7건` 으로
   훑었더니 `27건`(≤98ms 표본 수)이 걸렸다 — 단어 경계(`(?<![0-9])7건`)로 재검사해 0 임을 확인했다.
   스윕 결과는 **한 번 더 좁혀 확인**해야 하며, 이건 스윕을 반박하지 않고 **스윕에도 절차가 필요하다**는 뜻이다.
