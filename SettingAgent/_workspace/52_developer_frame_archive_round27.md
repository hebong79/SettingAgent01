...
## 8-5. 유닛테스트 커버리지 — **정직 고지**(CLAUDE.md 규칙 2)

27-C 가 자기 도구의 무테스트를 고지한 것을 보고 제 것도 훑었더니, **"테스트 있음"으로 뭉뚱그리면 안 되는 구분**이 있었다.
파일명 패턴이 아니라 **내용 기준**(`grep -rl` 로 실제 import 참조)으로 셌다:
...
**★ 27-C 와의 교차 확인 — 실카 경로가 가장 얇다(리더 인계 항목)**
27-C 가 자기 도구가 부르는 코어 11개를 함수 단위로 세어 **0 인 함수는 없으나 얇은 곳이 셋**이라고 보고했고,
공교롭게 **그 셋이 전부 실카 경로의 핵심**이다: `readGroundSpec`(설치고·화각표 해석) **1** ·
`interpolateHfov`(화각 보간) **2** · `detectBaysWithModel`(격자 산출) **2**.

이 중 **`readGroundSpec` 은 내 파일(`roiAuto.ts`)의 export** 라 내가 독립 확인했다 — **호출 테스트 1개**(`roiAutoSource.test.ts`).
두 에이전트가 **다른 방법으로 세어 같은 수**를 얻었다.

의미: 27-C 의 실카 지면모델 결론과 내 실카 아카이브 메타(설치고 13m·화각 19.231°·f 5666.5478117581315)가
**둘 다 저 얇은 함수들 위에 서 있다.** 시뮬 경로(`buildGroundInputs` 14 · `estimateGroundModels` 12)와 대비된다.
**이번 회차 결론을 뒤집는 근거는 아니지만**(라이브 재현 11/11 · 골든 비트 동일이 별도 증거다), 실카 전이가 본격화되면
**여기부터 두꺼워져야 한다**는 신호다. 리더 인계 항목으로 남긴다.
...
**이번 범위 밖이라 하지 않았고**, 다음 회차 항목으로 §10 에 넣었다.
...
4. **`roiAutoReplay` 의 행위 유닛테스트를 올려라**(§8-5) — 지금은 라이브 11/11 과 정적 봉인뿐이고, 픽스처 1장으로 도는 테스트가 최소 작업이다.
5. **아카이브를 켠 채로 vitest 를 돌리지 마라** — 설정으로 막았지만, 새 테스트 러너/스크립트를 만들면 같은 구멍이 다시 난다(F1).
...
6. **★ 「자기 것은 못 잡는다」는 틀렸다 — 못 잡는 것은 *다시 읽기*다.**
   ...
   **★ 마지막 자기적용 — 납품한 봉인 테스트의 양성 대조**(27-C 가 자기 부정단언에 양성대조를 붙인 것을 보고).
   `test/frameArchive.test.ts` 의 봉인 단언 3개는 **「0건이면 통과」 모양**이라, 패턴이 틀리면 **조용히 공허하게 통과**한다.
   실제 소스는 건드리지 않고 **사본에 위반을 주입**해 검출력을 실측했다(`git status -- src/` 공백으로 무변경 확인):

   | 단언 | 주입한 위반 | 결과 |
   |---|---|---|
   | 재현기가 검출 코어를 재구현하지 않음 | `detectBaysWithModel` 참조 추가 | **검출 ✔** |
   | 아카이브가 정본·DB 를 import 하지 않음 | `import { SqliteStore }` 추가 | **검출 ✔** |
   | `roiAuto` 의 archive 종료경로 3개 | 종료경로 1개 제거 | **검출 ✔** |

   **공허성 점검**: import 추출기가 실제로 4줄을 뽑는다(0 이면 술어 자체가 공허). → 봉인 3건 **전부 살아 있다.**
   부정 단언은 이렇게 **양성 대조를 붙여야** 비로소 증거가 된다 — 안 붙이면 "테스트가 green" 이 "검사를 안 했다"와 구별되지 않는다.
