...
**미해소 불일치 1건(정직)**: `jpeg 완료 → getptzfpos **시작**` 의 중앙값을 나는 **17ms**, 27-C 는 **31ms** 로 셌다. min 3 · max 335 는 양쪽이 정확히 같다. 원인 미규명(짝지음 후 `prev` 리셋 여부 차이로 추정). **부차 통계이고 결론에 개입하지 않으므로 어느 쪽도 단정하지 않는다** — 인용해야 하면 min/max 만 쓸 것.
...
| 5 | 27-C | 표본 2개로 `72~368ms` 단정 · 컷오프 200ms 동치 주장 | 27-B |
| 6 | 27-B | 중앙값 "약 61" 근사 (정확값 60) | 27-C |

**서로 검산하지 않았으면 여섯 개가 전부 회차 보고서에 남았다.** 특히 4·6 은 내가 낸 것이고, 4 는 「값과 함께 무엇을 주장하면 안 되는지를 남겨라」(§11-4)를 쓴 **바로 그 사람이 낸 양 혼동**이라 더 뼈아프다. 규율을 문서에 적는 것과 지키는 것은 다르다.

**일반화 가능한 규칙 2개**:
1. **로그에서 「간격」을 주장하려면 `time`(완료시각)끼리 빼라.** `ms` 는 소요시간이다. 이 저장소의 패킷 로그는 두 값을 같은 줄에 담아 혼동하기 쉽다.
2. **표본 2개로 범위를 단정하지 마라.** n=2 는 최악값을 상한으로 올려 대표성을 잃는다. 이 저장소는 로그가 남아 있으므로 전량 재추출 비용이 거의 0 이다.
