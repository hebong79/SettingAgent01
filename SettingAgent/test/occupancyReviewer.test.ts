...
/**
 * 검증자(qa-tester): OccupancyReviewer (설계서 §3.6, 성공기준 3·4 — DB 스키마 개편 후 재작성).
 * 전 프리셋 순회(캡 없음) → brain.judgeOccupancy → occByPreset(인메모리 Map, 구 store.insertOccupancy 대체).
 * 폴백 없음: LLM 불가/실패 시 graceful skip(저장 생략) + llmUnavailable 플래그(잡 미중단).
 * 경계 교차: judgeOccupancy 반환(spaces/occupiedCount/total/rate/confidence) → occByPreset 값 그대로.
 */
...
describe('OccupancyReviewer graceful skip (LLM 불가 — 성공기준 4)', () => {
  it('brain 미주입 → occByPreset 저장 0, llmUnavailable:true', async () => {
    const occByPreset = new Map<string, OccupancyJudgment>();
    const r = new OccupancyReviewer({});
    const res = await r.review(1, framesOf(['1:1', '1:2']), occByPreset);
    expect(res).toEqual({ llmUnavailable: true });
    expect(occByPreset.size).toBe(0);
  });
  ...
  it('brain.enabled=false → 저장 0, llmUnavailable:true(시도 안 함)', async () => {
    ...
    const r = new OccupancyReviewer({ brain });
    const res = await r.review(1, framesOf(['1:1']), occByPreset);
    expect(res).toEqual({ llmUnavailable: true });
    expect(occByPreset.size).toBe(0);
...
describe('OccupancyReviewer 정상 저장 (성공기준 3)', () => {
  it('전 프리셋 저장(캡 없음): 13 프레임 → occByPreset 13건(floor 의 12 캡과 대비)', async () => {
    const occByPreset = new Map<string, OccupancyJudgment>();
    const brain = fakeBrain(async () => judgment([{ id: 1, occupied: true }, { id: 2, occupied: false }]));
    const r = new OccupancyReviewer({ brain });
    const keys = Array.from({ length: 13 }, (_, i) => `1:${i + 1}`);
    const res = await r.review(7, framesOf(keys), occByPreset);
    expect(res).toEqual({ llmUnavailable: false });
    expect(occByPreset.size).toBe(13); // maxPerCheckpoint 캡 미적용(결정 3)
  });

  it('저장값 shape 교차검증: judgment 가 occByPreset 에 그대로 저장됨(camIdx/presetIdx 는 키에서 파생)', async () => {
    ...
    const r = new OccupancyReviewer({ brain });
    await r.review(5, framesOf(['1:2']), occByPreset);
    expect(occByPreset.size).toBe(1);
    const j = occByPreset.get('1:2'); // key '1:2' → camIdx 1, presetIdx 2(경계: split(':').map(Number))
    expect(j).toBeDefined();
    expect(j!.occupiedCount).toBe(2);
    expect(j!.total).toBe(3);
    expect(j!.rate).toBeCloseTo(2 / 3);
    // spaces 는 배열 그대로 보존(box 포함 향후 오버레이용) — JSON 문자열이 아님(구 spacesJson 폐기).
    expect(j!.spaces).toEqual([
  ...
  it('빈 spaces(total=0) → rate=0 저장(빈 화면 0% 충족)', async () => {
    ...
    expect(res).toEqual({ llmUnavailable: false });
    expect(occByPreset.size).toBe(1);
    const j = occByPreset.get('1:1')!;
    expect(j.total).toBe(0);
    expect(j.occupiedCount).toBe(0);
  ...
  it('expectedByPreset 전달 시 judgeOccupancy input.expected 채워짐', async () => {
    ...
    const r = new OccupancyReviewer({ brain });
    await r.review(1, framesOf(['1:1', '2:3']), occByPreset, undefined, { '1:1': 4, '2:3': 7 });
...
describe('OccupancyReviewer 부분 실패·정지 (성공기준 4)', () => {
  it('일부 프리셋 throw → 해당만 스킵·나머지 저장, succeeded>0 → llmUnavailable:false', async () => {
    ...
    const r = new OccupancyReviewer({ brain });
    const res = await r.review(1, framesOf(['1:1', '1:2', '1:3']), occByPreset);
    expect(res).toEqual({ llmUnavailable: false }); // 최소 1건 성공
    expect([...occByPreset.keys()].sort()).toEqual(['1:1', '1:3']); // preset2 스킵, 1·3 저장
  });
  ...
  it('shouldStop=()=>true → 첫 프리셋 전 break, 저장 0', async () => {
    ...
    const r = new OccupancyReviewer({ brain });
    const res = await r.review(1, framesOf(['1:1', '1:2']), occByPreset, () => true);
    expect(occByPreset.size).toBe(0);
    expect(brain.judgeOccupancy).not.toHaveBeenCalled();
...
describe('OccupancyReviewer 관측성 로깅 (T6 — null vs throw 구분 + 카운트)', () => {
  it('전 프리셋 null → 프리셋별 null warn + 요약 nullCount 집계(errorCount=0)', async () => {
    ...
    const brain = fakeBrain(async () => null);
    const r = new OccupancyReviewer({ brain });
    await r.review(1, framesOf(['1:1', '1:2']), occByPreset);
