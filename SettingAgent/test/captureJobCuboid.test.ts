...
/** 프로덕션 `CaptureJob` 1라운드 실행. `getSnapshot().dets` 로 인메모리 적재분을 그대로 읽는다(구 insertDetections 인자 캡처 대체). */
...
describe('★★ T6 — 육면체 on/off 가 점유 판정 경로를 바꾸지 않는다(Goal 3 · 회귀 0)', () => {
  it('`getSnapshot().dets` 가 **완전히 동일**하다(deep equal) — 육면체 off vs on', async () => {
    const off = await runOneRound(); // cuboidCtx 미주입 → 육면체 전 기능 off.
    const on = await runOneRound({ cuboidCtx: async () => CTX }); // 육면체 on(매 라운드 seg 호출).

    // ★ 봉인: 검출 인메모리 적재분(구 insertDetections 인자 대체)이 **비트 동일**.
    expect(on.job.getSnapshot().dets).toEqual(off.job.getSnapshot().dets);
    expect(off.job.getSnapshot().dets.length).toBeGreaterThan(0); // 공허한 통과 방지(둘 다 비지 않음).
  ...
  it('`aggregate()` 산출(점유의 실제 소비처)도 동일하다', async () => {
    ...
    const agg = (r: Awaited<ReturnType<typeof runOneRound>>) => {
      const snap = r.job.getSnapshot();
      return aggregate(snap.dets, snap.presetRounds, {
...
describe('★ 잡 사망 금지 — seg 가 죽어도 수집은 계속된다(마스터 §5)', () => {
  ...
  it('★ `cuboidCtx` 콜백 자체가 throw 해도(파일 IO 예외 등) 잡은 죽지 않고 검출은 그대로 적재된다', async () => {
    ...
    const dets = r.job.getSnapshot().dets;
