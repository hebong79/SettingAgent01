...
describe('FK 스킵 — preset_info 부모 없는 (cam,preset)', () => {
  ...
  // ROI 파일이 정의한 (cam,preset) 은 preset_info 자리표시자(PTZ 미상)를 만들어 **전량 적재**한다.
  it('부모 없는 (cam,preset) 은 자리표시자를 만들어 전량 INSERT 된다(skipped 없음)', () => {
    const s = newStore();
    const dir = newTmp();
    s.upsertPlaceInfo([placeRow]);
    s.upsertCameraInfo([cameraRow]);
    s.upsertPresetInfo([presetRow]); // cam1:preset1 만 실측 부모 존재
  ...
  it('자리표시자는 camerapos 실측 PTZ 를 덮어쓰지 않는다', () => {
    const dir = newTmp();
    const dbPath = join(dir, 'fk.sqlite'); // preset_info 직접 검사를 위해 파일 DB(:memory: 는 2차 연결 불가).
    ...
    const rows = db.prepare('SELECT cam_id, preset_id, preset_name, pan, tilt, zoom FROM preset_info').all() as Array<
      { cam_id: number; preset_id: number; preset_name: string | null; pan: number; tilt: number; zoom: number }
    >;
    db.close();
    expect(rows.find((r) => r.cam_id === 1 && r.preset_id === 1)).toMatchObject({ preset_name: '실측', pan: 51.5, tilt: 9.3, zoom: 14.4 });
  ...
  it('ROI 파일이 프리셋 PTZ 를 담고 있으면 자리표시자 없이 실값으로 부모가 선다', () => {
    ...
    const db = new Database(dbPath, { readonly: true });
    const rows = db.prepare('SELECT cam_id, preset_id, preset_name, pan, tilt, zoom FROM preset_info ORDER BY preset_id').all();
    db.close();
    expect(rows).toEqual([
      { cam_id: 2, preset_id: 1, preset_name: 'C2-P1', pan: 139, tilt: 17, zoom: 2.5 },
      { cam_id: 2, preset_id: 2, preset_name: null, pan: 150.5, tilt: 20, zoom: 3 },
    ]);
  });
